...
const FileExplorer = ({ options }) => {
  ...
  const handleDownloadFile = async fileKeys => {
    fileKeys.forEach(async fileKey => {
      ...
      const blob = await fetcher(
        datastore.getObjectDownloadUrlByFilekey(fileKey)
      );
  ...
  return (
    <>
      <LaunchWorkflow options={{ workflowId: options?.workflowId }} />
      <FileBrowser
        files={files}
        ...
        noFilesMessage={
          loadingFiles ? (
            ...
            <div className="spinner-container">No files.</div>
          )
        }
      />
    </>
